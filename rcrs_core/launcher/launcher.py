...
from rcrs_core.constants.constants import (
...
class Launcher:
    ...
    def run(self, kwargs):
        ...
        for agn, num in agents.items():
            for _ in range(num):
                ...
                process = Process(
                    target=self.launch, args=(eval(agn)(precompute), request_id)
                )
                process.start()
    ...
    def pars_args(self, sys_args: List[str]) -> dict:
        ...
        try:
            elements["port"] = (
                int(args.get("-p")) if "-p" in args else int(DEFAULT_KERNEL_PORT_NUMBER)
            )
