...
class PoliceForceAgent(Agent):
    ...
    def think(self, time_step, change_set, heard):
        ...
        if time_step == self.config.get_value(
            kernel_constants.IGNORE_AGENT_COMMANDS_KEY
        ):
            ...
            if isinstance(en, Road):
                for b in en.get_blockades():
                    ...
                    if blockade:
                        ...
                        if distance < best_distance and distance < float(
                            self.config.get_value("clear.repair.distance")
                        ):
